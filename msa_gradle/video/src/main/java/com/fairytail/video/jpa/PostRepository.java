...
public interface PostRepository extends JpaRepository<PostEntity, Long> {
    ...
    List<PostEntity> findTop25ByLatAndLngAndStatusOrderByDateDesc(Double lat, Double lng); //상위 25개를 lat, lng와 0.01 차이나고 status가 0인 것들로 날짜 순으로 조회
    ...
    List<PostEntity> findTop25ByLatAndLngAndStatusOrderByLikeCntDesc(Double lat, Double lng); //상위 25개를 lat, lng와 0.01 차이나고 status가 0인 것들로 좋아요 순으로 조회

    List<PostEntity> findAllByStatus(Integer status); //공개된 모든 게시물을 리스트로 조회
}
